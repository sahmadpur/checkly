...
export const STATUS_FILTERS = ["", "OPEN", "OVERDUE", "SUBMITTED", "APPROVED", "REJECTED"] as const;
...
export function ChecklistList({ basePath, instances, filter, subtitle, empty }: {
  ...
  const t = useTranslations("checklists.list");
  const locale = useLocale();
  return (
    <>
      <nav aria-label={t("filterLabel")} className="-mx-4 flex gap-1.5 overflow-x-auto px-4 pb-1 text-sm [scrollbar-width:none] md:mx-0 md:flex-wrap md:px-0">
        {STATUS_FILTERS.map((value) => (
          <Link key={value} href={value ? `${basePath}?status=${value}` : basePath}
            aria-current={filter === value ? "page" : undefined}
            className={cn("shrink-0 rounded-full px-3 py-1.5 font-medium transition-colors", filter === value ? "bg-primary text-primary-foreground" : "bg-card text-muted-foreground ring-1 ring-border hover:text-foreground")}>{t(`filter.${value || "all"}`)}</Link>
        ))}
      </nav>
      <List>
        {instances.length === 0 && <ListEmpty>{filter ? t("emptyFiltered") : empty}</ListEmpty>}
        {instances.map((i) => (
          <ListLink key={i.id} href={`/checklists/${i.id}`} rail={STATUS_RAIL[statusKey(i.status, i.overdue)]}>
            ...
            <span className="flex items-center gap-2 text-muted-foreground">
              <LocalTime iso={i.dueAt.toISOString()} fallback={formatDateTime(i.dueAt, locale)} />
