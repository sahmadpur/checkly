...
export function AnswerView({ items, mediaUrls, downloadUrls }: { items: InstanceItemRow[]; mediaUrls: Record<string, string>; downloadUrls: Record<string, string> }) {
  const t = useTranslations("checklists.detail");
  return (
        ...
        return (
          <li key={i.id} className="rounded-xl bg-card p-4 text-sm ring-1 ring-border">
            <div className="flex items-center justify-between gap-3">
              ...
              {!answered && <span className="shrink-0 text-xs text-muted-foreground">{t("notAnswered")}</span>}
            </div>
            {answered && (
              <div className="mt-2">
                {i.type === "CHECKBOX" && i.checked && <span className="inline-flex items-center gap-1.5 font-medium text-success-foreground"><Check className="size-4" aria-hidden /> {t("done")}</span>}
                ...
                {i.type === "PHOTO" && i.fileKey && (mediaUrls[i.id]
                  ...
                  : <span className="text-muted-foreground">{t("photoUnavailable")}</span>)}
                {i.type === "VIDEO" && i.fileKey && (mediaUrls[i.id]
                  ? <div className="space-y-1"><video controls playsInline src={mediaUrls[i.id]} className="max-h-80 w-full rounded-lg bg-black" /><a className="text-xs underline underline-offset-4" href={downloadUrls[i.id] ?? mediaUrls[i.id]}>{t("downloadVideo")}</a></div>
                  : <span className="text-muted-foreground">{t("videoUnavailable")}</span>)}
              </div>
            )}
          </li>
        );
