...
export function ScheduleForm({ propertyId, templates, workers, todayYmd, templateArchived, schedule }: {
  ...
  return (
    <form
      ...
      className="space-y-8"
    >
      <Section title="What and who" card>
        <div className="space-y-4">
          <div className="space-y-1.5">
            ...
            {templateArchived && <Notice tone="warning">This template is archived. Pick another to keep generating checklists.</Notice>}
          </div>
      ...
      <Section title="When" card>
        <div className="space-y-4">
          <fieldset className="space-y-3">
            <legend className="mb-1 text-sm font-medium">Repeats</legend>
            <div className="flex rounded-lg bg-muted p-1">
              {(["DAILY", "WEEKLY", "MONTHLY"] as const).map((f) => (
                <label key={f} className={segment}><input type="radio" name="freq" value={f} checked={freq === f} onChange={() => setFreq(f)} className="sr-only" />{f[0] + f.slice(1).toLowerCase()}</label>
              ))}
            </div>
            {freq === "WEEKLY" && (
            ...
            {freq === "MONTHLY" && (
              <div className="space-y-1.5"><Label htmlFor="dayOfMonth">Day of month</Label><Input id="dayOfMonth" name="dayOfMonth" type="number" inputMode="numeric" min={1} max={31} defaultValue={schedule?.dayOfMonth ?? 1} className="w-24" required /></div>
            )}
          </fieldset>
          <div className="grid gap-4 sm:grid-cols-3">
            <div className="space-y-1.5"><Label htmlFor="dueTime">Due time</Label><Input id="dueTime" name="dueTime" type="time" defaultValue={schedule?.dueTime ?? "09:00"} required /></div>
            <div className="space-y-1.5"><Label htmlFor="startsOn">Starts on</Label><Input id="startsOn" name="startsOn" type="date" defaultValue={schedule ? ymd(schedule.startsOn) : todayYmd} required /></div>
            <div className="space-y-1.5"><Label htmlFor="endsOn">Ends on</Label><Input id="endsOn" name="endsOn" type="date" defaultValue={schedule?.endsOn ? ymd(schedule.endsOn) : ""} /></div>
          </div>
        </div>
      </Section>

      <FormError message={error} />
      {saved && schedule && <FormSuccess message="Saved. Changes apply to future occurrences." />}
      <SubmitButton pending={pending}>{schedule ? "Save schedule" : "Create schedule"}</SubmitButton>
    </form>
  );
}
