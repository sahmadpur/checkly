...
function Section({ title, items, locale }: { title: string; items: InstanceSummary[]; locale: string }) {
...
export default async function TodayPage() {
  ...
  const [t, locale] = await Promise.all([getTranslations("checklists.today"), getLocale()]);
  const b = bucket(await listMine(ctx));
  const empty = Object.values(b).every((x) => x.length === 0);
  return (
    <div className="space-y-6">
      <PageHeader title={t("title")} description={new Intl.DateTimeFormat(locale, { weekday: "long", month: "long", day: "numeric" }).format(new Date())} />
      {empty && (
      ...
      <Section title={t("overdue")} items={b.overdue} locale={locale} />
      <Section title={t("rework")} items={b.rework} locale={locale} />
      <Section title={t("dueToday")} items={b.today} locale={locale} />
      <Section title={t("upcoming")} items={b.upcoming} locale={locale} />
      <Section title={t("done")} items={b.done} locale={locale} />
    </div>
  );
}
