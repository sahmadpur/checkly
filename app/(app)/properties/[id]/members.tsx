...
export function PropertyMembers({ propertyId, members, candidates, canEdit }: {
  ...
  const t = useTranslations("properties.members");
  const [error, setError] = useState<string | null>(null);
  const [pending, start] = useTransition();
  const act = (fn: () => Promise<{ ok: boolean; error?: string }>) =>
    start(async () => { const r = await fn(); if (!r.ok) setError(r.error ?? t("failed")); });

  return (
    <Section title={t("title")} description={canEdit ? t("description") : undefined}
      actions={canEdit && candidates.length > 0 && (
        <Select aria-label={t("addLabel")} className="w-44 [&>select]:h-9" disabled={pending} value=""
      ...
      <List>
        {members.length === 0 && <ListEmpty>{t("empty")}</ListEmpty>}
        {members.map((m) => (
          <ListRow key={m.userId}>
            ...
            {canEdit && (
              <Button variant="ghost" size="sm" disabled={pending}
                onClick={() => act(() => removePropertyMemberAction(propertyId, m.userId))}>{t("remove")}</Button>
            )}
          </ListRow>
        ))}
      </List>
