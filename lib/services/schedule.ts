...
export function validateRule(i: ScheduleInput) {
  if (i.freq === "WEEKLY") {
    ...
    if (days.length < 1 || days.length > 7 || days.some((d) => !Number.isInteger(d) || d < 0 || d > 6)) throw invalid("pickWeekday");
    if (i.dayOfMonth != null) throw invalid("weeklyNoDayOfMonth");
  } else if (i.freq === "MONTHLY") {
    if (i.dayOfMonth == null || !Number.isInteger(i.dayOfMonth) || i.dayOfMonth < 1 || i.dayOfMonth > 31) throw invalid("dayOfMonthRange");
    if (i.daysOfWeek.length) throw invalid("monthlyNoWeekdays");
  } else {
    if (i.daysOfWeek.length || i.dayOfMonth != null) throw invalid("dailyNoDays");
  }
  if (!TIME.test(i.dueTime)) throw invalid("dueTimeFormat");
  if (i.endsOn && i.endsOn.getTime() < i.startsOn.getTime()) throw invalid("endBeforeStart");
  if (i.assigneeIds.length === 0) throw invalid("pickWorker");
}
...
async function loadForManager(ctx: Ctx, id: string) {
  ...
  if (!s) throw notFound("scheduleNotFound");
...
async function assertInputs(ctx: Ctx, propertyId: string, input: ScheduleInput) {
  ...
  if (members !== ids.length) throw invalid("assigneeNotMember");
  return { tpl, ids };
}
